	...
	if ((1)); then
		...
		for P in $(ls $SPLIT_PATTERN*); do
			...
			echo "$CLIPPED_P: $P.bam
				@$0 $PARAM_MULTITHREADING --bam=$P.bam --manifest=$MANIFEST --threads=1 --output=$CLIPPED_P --env=$ENV --clipping_options='$CLIPPING_OPTIONS' --ref=$REF; #--verbose;
				@rm -f $P.bam
			" >>$MK;
...
if ((1)); then
	...
	BED_PRIMERS_1_BASED=$BED_PRIMERS.1_based.bed
	...
	TMP_FILES=$TMP_FILES" $BED_PRIMERS_1_BASED $BED_PRIMERS_FASTA $BED_PRIMERS_OPTIONS $BEDTOOLS_OUTPUT $BEDTOOLS_ERR $BEDTOOPTION_OUTPUT $BEDTOOPTION_ERR"
	(($VERBOSE)) && echo -e "\n# BED 0-based to 1-based..."
	cat $BED_PRIMERS | awk '{print $1"\t"$2+1"\t"$3}' > $BED_PRIMERS_1_BASED
	(($VERBOSE)) && echo -e "\n# BED 1-based to FASTA..."
	#$BEDTOOLS getfasta -fi $REF -bed $BED_PRIMERS -fo $BED_PRIMERS_FASTA 1>$BEDTOOLS_OUTPUT 1>$BEDTOOLS_ERR
	$BEDTOOLS getfasta -fi $REF -bed $BED_PRIMERS_1_BASED -fo $BED_PRIMERS_FASTA 1>$BEDTOOLS_OUTPUT 1>$BEDTOOLS_ERR
